...
class HDF5Dataset(Dataset):
    ...
    def _concatenate_frames(self, sets, group):
                ...
                for frame in range(0, d_len, self.step_size):
                    ...
                    if frame + self.frame_count > d_len:
                        ...
                        if self.data is None:
                            ...
                            self.data=np.concatenate((self.data,seq))
        ...
        print()
        print(f'The number of {group} labels {len(self.data)}', end='\r')
        print()
            ...
            for frame in range(0, frame_length, self.step_size):
                if frame + self.frame_count > l.shape[0]:
                    ...
                    if self.labels is None:
                        ...
                        self.labels = np.concatenate((self.data,seq))
                
            # progress bar
            i += 1
            print (f'Creating {group} labels {i}/{len(sets[group])}', end='\r')
        print()
        print(f'The number of {group} labels {len(self.labels)}', end='\r')
        print()
    ...
    def create_labels(self):
        ...
        with open(label_file_path, 'r') as l:
            ...
            if self.group == 'train':
                ...
                self.save_varibles( "data/var/", "train_data", "train_label")
            elif self.group == 'val':
                self._concatenate_labels(segments, 'validation')
                self.save_varibles( "data/var/", "val_data", "val_label")
            elif self.group == 'test':
                self._concatenate_labels(segments, 'test')
                self.save_varibles( "data/var/", "test_data", "test_label")
            else:
                raise Exception('Invalid group')
